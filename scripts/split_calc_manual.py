...
def load_local_event_data(split, event_dir, station_info, data_format="SAC", 
                        new_sampling_rate=10.0, verb=False):
    """
        ...
        for iev in ievs:
            ...
            if accept:
                ...
                if args.recalc:
                    ...
                    # --------------------------------------------------
                    # Loop over filter bands to find best SNR
                    # --------------------------------------------------
                    for fmin, fmax in args.filterbands_list:

                        # 使用副本，避免污染原始数据
                        data_test = split.dataLQT.copy()
                        ...
                        # 临时赋值，用于计算 SNR
                        split.dataLQT = data_test
                        split.calc_snr()

                        snrq = split.meta.snrq

                        if args.verb:
                            print(f"    Test band {fmin:.2f}-{fmax:.2f} Hz: SNRQ = {snrq:.2f}")
                    ...
                    # 使用 SNR 最优的滤波结果
                    # --------------------------------------------------
                    split.dataLQT = best_LQT
                    split.meta.snrq = best_snrq
                    split.meta.best_filter_band = best_band
                ...
                if args.calc or args.recalc:
                    ...
                    if args.diagplot:
                        if (split.null is False) and (split.quality in ["Good", "Fair"]):
                            ...
                            # ---------------------------------------------
                            if hasattr(split.meta, "best_filter_band"):
                                fmin, fmax = split.meta.best_filter_band
                                snrq = split.meta.snrq

                                fig.text(
                                    0.02, 0.02,
                                    f"Filter band: {fmin:.2f}–{fmax:.2f} Hz\nSNRQ = {snrq:.2f}",
                                    fontsize=9,
                                    ha="left",
                                    va="bottom"
                                )
